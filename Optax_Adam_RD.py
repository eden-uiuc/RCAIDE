...
dt = T / N  # time step
x6_lb = 0  # lower bound for x6
mass = 2  # mass (kg)
rho = 1  # disk radius (m)
I_d = 0.25*mass*rho**2  # dilateral moment of inertia (kgm^2)
I_a = 0.5*mass*rho**2  # axial disk moment of inertia (kgm^2)

# Initial conditions and target
x0 = jnp.array([0, 0, jnp.pi/2, 0, 0, 0])  # initial state
x_target = jnp.array([10, 10, jnp.pi / 4, 0, 0, 0])  # target state
...
def next_state(Xk, Uk, dt):
    ...
    u1, u2 = Uk
    # Dynamics
    x1_dot = rho * x6 * jnp.cos(x3)
    x2_dot = rho * x6 * jnp.sin(x3)
    x3_dot = x5
    x4_dot = x6
    x5_dot = u1 / I_d
    x6_dot = u2 / (I_a + (mass * rho**2))
...
def step(U, opt_state):
    ...
    loss, grads = value_and_grad(U, x0, dt, x_target)
    updates, opt_state = optimizer.update(grads, opt_state)
    U = optax.apply_updates(U, updates)

    # Project onto constraints
    U = project_constraints(U, x0, dt)
...
plt.axhline(y=x6_lb, color='r', linestyle='--', label='x6 lower bound')
plt.xlabel('Time')
plt.ylabel('States')
plt.legend()
